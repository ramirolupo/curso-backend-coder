...
routerCarts.post('/:id/products', (req, res) => {
	...
	if (!productSaved) return res.send({ message: 'Error' });
...
routerCarts.delete('/:id/products/:id_prod', (req, res) => {
	...
	if (isNaN(id) || isNaN(id_prod)) return res.send({ message: 'Ingresa el ID de un carrito listado' });
	const productDeleted = carts.deleteProduct(id, id_prod);
	if (productDeleted == -1 || !productDeleted) return res.send({ message: 'Error' });
